...
class HoovPlayer():
    ...
    def get_points(self,go):
        ...
        return white_groups,black_groups,whites,blacks,placements_w,placements_b,whites_l,blacks_l,whites_a,blacks_a,len(w_l),len(b_l)
    ...
    def get_successors(self,go,piece_type):
        ...
        #    en_groups = white_groups
        #    en_libs = whites_l2
        #    en_stones = whites
        #    my_libs = blacks_l2
        #    my_stones = blacks
        #else:
        #    placements = placements_w
        #    en_groups = black_groups
        #    en_libs = blacks_l2
        #    en_stones = blacks
        #    my_libs = whites_l2
        #    my_stones = whites
        ...
        for it in placements:
            ...
            #                board[k[0]][k[1]]=0
            #            en_groups.remove(i)
            #cp.update_board(board)
            
            died_pieces,en_pieces,my_pieces,en_libs = cp.make_move(it,piece_type)
            cp.last_move = it
            #cp.visualize_board()
            #cand['score']=(en_stones-my_stones-1)*2
            cand['score']=(en_pieces-len(died_pieces)-my_pieces)*2 + en_libs
            #cp.dieds = died_pieces
            #print("en_pieces: ",en_pieces," my_pieces ",my_pieces," en_libs: ",en_libs," len(died_pieces): ",len(died_pieces))
            #print("en_pieces: ",blacks," my_pieces: ",whites," en_libs ",blacks_l)
            cand['board']=counter
            #if piece_type == 1:
            #    cand['score']=blacks - whites + blacks_l
                #cand['score']=(blacks*4 +blacks_l )-((whites+2.5)*5+whites_l)
            #else:
                #cand['score']=((whites+2.5)*5+whites_l)-(blacks*5 +blacks_l )
            #    cand['score']=whites - blacks + whites_l
            hass[counter]=cp
            children.append(cp)
            scores.append(cand)
            counter +=1
